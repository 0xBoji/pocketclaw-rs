---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for PhoneClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, PhoneClaw cron scheduling for periodic checks, or version status checks on a machine running PhoneClaw (laptop, workstation, Pi, VPS).
---

# PhoneClaw Host Hardening

## Overview

Assess and harden the host running PhoneClaw, then align it to a user-defined risk tolerance without breaking access. Use PhoneClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim PhoneClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. PhoneClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if PhoneClaw can access sensitive data.
...
### 2) Run PhoneClaw security audits (read-only)

As part of the default read-only checks, run `phoneclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `phoneclaw security audit` (faster, non-probing)
2. `phoneclaw security audit --json` (structured output)

Offer to apply PhoneClaw safe defaults (numbered):

1. `phoneclaw security audit --fix`

Be explicit that `--fix` only tightens PhoneClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check PhoneClaw version/update status (read-only)

As part of the default read-only checks, run `phoneclaw update status`.
...
- Credential hygiene notes (location of PhoneClaw creds, prefer disk encryption)
...
- PhoneClaw security audit (re-run)
...
After PhoneClaw install or first hardening pass, run at least one baseline audit and version check:

- `phoneclaw security audit`
- `phoneclaw security audit --deep`
- `phoneclaw update status`

Ongoing monitoring is recommended. Use the PhoneClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `phoneclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `phoneclaw update status`
...
Before creating, `phoneclaw cron list` and match on exact `name`. If found, `phoneclaw cron edit <id> ...`.
If not found, `phoneclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `phoneclaw update status` (preferred for source checkouts and channels)
2. `npm view phoneclaw version` (published npm version)

## PhoneClaw command accuracy

Use only supported commands and flags:

- `phoneclaw security audit [--deep] [--fix] [--json]`
- `phoneclaw status` / `phoneclaw status --deep`
- `phoneclaw health --json`
- `phoneclaw update status`
- `phoneclaw cron add|list|runs|run`

Do not invent CLI flags or imply PhoneClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by PhoneClaw compaction:
